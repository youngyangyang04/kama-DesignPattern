...
public:
    static ShoppingCartManager *getInstance() {
        ...
        //双重检查加锁
        if (shoppingCartManager == nullptr) {
            ShoppingCartManager::mut.lock();
            if (shoppingCartManager == nullptr) {
                shoppingCartManager = new ShoppingCartManager;
            }
            ShoppingCartManager::mut.unlock();
        }
//        Singleton::mut.unlock();
    ...
    void addObject(std::string name, int num) {
        auto iter = std::find_if(lists.begin(), lists.end(), [&name](const std::pair<std::string, int> &p) {
            return p.first == name;
        });
        lists.emplace_back(name, num);
    }
...
// 初始化静态成员变量
ShoppingCartManager *ShoppingCartManager::shoppingCartManager = nullptr;
std::mutex ShoppingCartManager::mut;


int main(int argc, char *argv[]) {

    ShoppingCartManager *instance;
    instance = ShoppingCartManager::getInstance();
    std::string name;
    int num;

    while (std::cin >> name >> num) {
        instance->addObject(name, num);
    }

    std::vector<std::pair<std::string, int> > l = instance->getlist();
    for (auto p: l) {
